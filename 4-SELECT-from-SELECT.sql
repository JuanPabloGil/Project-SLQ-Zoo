...
WHERE area >= ALL
(SELECT area FROM world AS y WHERE y.continent=x.continent AND population>0);
...
WHERE population > ALL(SELECT population*3 FROM world AS y WHERE y.continent = x.continent AND y.name != x.name);
